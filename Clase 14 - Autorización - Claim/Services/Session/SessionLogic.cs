...
{
    // Clase 12: Aunque se ha segregado la interface ILogin de ISession ambas se implementan en la misma clase
    public class SessionLogic : ISessionLogic, ILoginLogic
    {
        ...
        public SessionItem GetById(long id)
        {
            throw new NotImplementedException();
        }

        LoginResponse ILoginLogic.Login(LoginRequest login)
        {
            ...
            IEnumerable<Claim> claims = new[]
            {
                ...
                // Clase 14: En nuestro caso los roles son separados por coma, por lo tanto se agregan luego en un bucle
                //new Claim("role", user.Role ?? ""),
                //new Claim("role", user.Role ?? ""),
                //new Claim("role", user.Role ?? ""),
            };

            // Clase 14: Se agregan los roles desde un texto separado por coma
            user.Role?.Split(",").ToList().ForEach(role => claims = claims.Append(new Claim("role", role)));
