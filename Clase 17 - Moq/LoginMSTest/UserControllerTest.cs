...
namespace LoginMSTest
{
    ...
    public class UserControllerTest
    {
        ...
        public void Create_con_Moq()
        {
            ...
            var moqUserEntity = new Mock<IUserEntity>();
            //moqUserEntity.Setup(ue => ue.GetForUsername(request.Username)).Returns((UserItem?)null);
            moqUserEntity.Setup(ue => ue.Create(It.IsAny<UserCreateDTO>())).Returns(request.ToUserCreateDTO().ToUserItem());

            IUserLogic userLogic = new UserLogic(moqUserEntity.Object);

            UserController userController = new UserController(logger, userLogic, configuration);
